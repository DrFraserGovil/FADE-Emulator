# To-Do List

As this is code under active, heavy development, we're keeping the to-do list fairly prominent. We have also used [todo notes](https://github.com/folke/todo-comments.nvim) scattered throughout the code for warnings and fix messages; hence the standardised format. 

### Today's Tasks

* TODO: Extract into a library + interface?
* TODO: Add a simple clusterer to the input data to allow for more efficient training 
* TODO: Create the scoring system 
* TODO: Basic annealing optimisation 

### This Week's Tasks
* TODO: Get the inference routine up 
* TODO: Create visualisation / interface routine

### Longer Term Tasks 

* TODO: Bones of the more intelligent optimiser
* TODO: Documentation setup


## Completed Taks

* ~Create single-model inference interface~
* ~Basic training data readin~
* ~Simple synthetic data for testing~
* ~Get an inference routine to read the parameter from a vault~
* ~Generalise the FADE model to be multidimensional from the get-go, rather than retrofittign it in~
* ~Get a parameter model to save itself to a suitable vault~
* ~Design a standardised vault system for storing the output of a training routine~
* ~Unpack interface~
* ~Create JSL/LSJ interface~
* ~Parameter vector indexing~
