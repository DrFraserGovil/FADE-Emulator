\section{The Splat-Experts}
        ...
        In the previous model, each expert (at a position $\vec{x} = \vec{c}_i$) was equipped with a single PDF. In this model, we equip them with $N_m$ gaussels, each of which has a weight, $\alpha_j$, a mean $\vec{\mu}_j \in \mathbb{R}^p$ and a variance $\vec{\Sigma}_j \in \mathbb{R}^{p\times p}$. 

        Each expert is therefore makes a prediction of a latent field, $\ell_j$:
        
        \begin{equation}
            \ell_i(\vec{z}) = \sum_j^{N_m} \alpha_j \mathcal{N}(\vec{z} | \vec{\mu}_j, \Sigma_j)
        \end{equation}

        \begin{aside}[The Simple Approach]
            This quantity $\ell_i$ \textit{could} be converted into a probability distribution function $p_i(\vec{z}) = h(y | g^{-1}(\ell_i(\vec{z})))$. This can then be used as $p_i$ in the standard LASPE formulation, requiring almost no modifications. 

            However, in order to build up a map of predictions across $\vec{z}$, you would need to evaluate the posterior predictive at every point on the mesh. This is extremely costly.             
        \end{aside}
        ...
        \begin{align}
            \begin{pmatrix}\alpha_j^\text{con}(\vec{x}) \\ \vec{\mu}_j^\text{con}(\vec{x}) \\ \vec{\Sigma}_j^\text{con}(\x)\end{pmatrix} 
            & = 
            \sum_{k=1}^{N_d}\sum_{i=1}^{N_e} T_k(\vec{x}) w_i^k(\x)   \begin{pmatrix}\alpha_j^i\\ \vec{\mu}_j^i\\ \vec{\Sigma}_j^i\end{pmatrix}
        \end{align}
        The consensus splat field is then:
        \begin{equation}
            \ell_\text{con}(\vec{z},\vec{x}) = \sum_j^{N_m} \alpha_j^\text{con}\mathcal{N}(\vec{z} | \vec{\mu}^\text{con}_j, \Sigma_j^\text{con})
        \end{equation}
        
        Given a final emulated PDF of:

        \begin{equation}
            p(y | \vec{x},\vec{z},\vT,\vDelta,\h) = h(y | g^{-1}(\ell_\text{con}(\vec{z},\vec{x} | \vT)))
        \end{equation}
