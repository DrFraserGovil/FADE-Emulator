\section{The Data \& The Mechanistic Model}
    ...
    We suppose that there exists some observable $\y \in \R^q$ which may be extracted from a function $\hat{M}$ evaluated at a parameter vector $\vec{P} \in \vec{\Pi}$:
    \begin{equation}
        \y = \hat{M}(\vec{P})
    \end{equation}
    ...
    \subsubsection*{`Semi-mechanistic' Models}
        ...
        \[\y \sim Y\]
        Despite this, the model is a `principled construction': derived from an axiomatic model of how the underlying reality functions. 
